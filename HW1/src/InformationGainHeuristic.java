 ...
 * B-Number:    XXX
 */
public class InformationGainHeuristic implements DecisionTree {
    ...
    @Override
    public void validate(DataSet validationDataSet) {
    }

    @Override
    public void test(DataSet testingDataSet) {
        ...
        System.out.println("Accuracy: " + correctPredictions + "/" + total +
                " = (" + ((double) correctPredictions * 100 / total) + "%)");
    }
    ...
    private Node id3(DataSet trainingDataSet) {
        ...
        for (String vi : trainingDataSet.getAttributeDistinctValues().get(decisionAttribute)) {
            ...
            /*  Get Attributes - decisionAttribute */
                ...
                node.getChildren().put(vi, id3(dataSetFiltered));
            }
        }
    ...
    private int getBestClassifier(DataSet trainingDataSet) {
        ...
        double overallEntropy = getEntropy(trainingDataSet);
        for (int i = 0; i < trainingDataSet.getAttributesCount() - 1; i++) {
            double gain = getGain(trainingDataSet, overallEntropy, i);
            gainList.add(gain);
        }
        return gainList.indexOf(Collections.max(gainList));
    }

    private double getEntropy(DataSet trainingDataSet) {
        Map<String, Integer> count = new HashMap<>();
        int targetAttribute = trainingDataSet.getTargetAttribute();
        for (String value : trainingDataSet.getAttributeDistinctValues().get(targetAttribute)) {
            count.put(value, 0);
        ...
        double result = 0;
        for (String value : count.keySet()) {
            double probability = (double) count.get(value) / total;
            result -= (probability * logToBase2(probability));
        }
        return result;
    }

    private double getEntropy(DataSet trainingDataSet, int attribute, String attributeValue) {
        Map<String, Integer> count = new HashMap<>();
        int targetAttribute = trainingDataSet.getTargetAttribute();
        for (String value : trainingDataSet.getAttributeDistinctValues().get(targetAttribute)) {
            count.put(value, 0);
    ...
    private double getGain(DataSet trainingDataSet, double overallEntropy, int attribute) {
        Map<String, Integer> count = new HashMap<>();
        ...
        double result = overallEntropy;
        for (String value : count.keySet()) {
            result -= ((double) count.get(value) / total) * getEntropy(trainingDataSet, attribute, value);
        }
        return result;
    }
